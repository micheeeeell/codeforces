...
signed main(){
    ...
    vector<ll> b(m);
    rep(i,0,m) cin >> b[i];
    if(*min_element(all(a)) < b[0] || a[n-1] < b[m-1]){
        cout << 0 << "\n";
        return 0;
    }
    vector<ll> pos(m), pos2(m);
    ll id = m-1, id2 = m-1;
    rrep(i,n,0){
        if(id >= 0 && a[i] == b[id]){
            pos[id] = i;
    ...
    SegmentTree<ll> seg(n, f, INF);
    seg.build(a);
    bool ok = true;
    ll l = 0;
    rep(i,0,m-1){
        ok &= seg.query(l, pos[i+1]) == b[i];
        debug(seg.query(l, pos[i+1]));
        l = pos[i+1];
    }
    ok &= seg.query(l, n) == b[m-1];
    if(!ok){
        cout << 0 << "\n";
        return 0;
    }
